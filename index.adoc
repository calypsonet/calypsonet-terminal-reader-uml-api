...
image::uml/card-reader-observation-fsm.svg[Card reader observation state machine,align=center]
...
* `[CARD_PROCESSABLE]` -- the inserted card is eligible for processing, *OR* card-presence notifications are enabled for all cards (`CardPresenceNotificationPolicy == ALWAYS`).
* `[CARD_NON_PROCESSABLE]` -- the inserted card is not eligible for processing, *AND* card-presence notifications are restricted to matched cards (`CardPresenceNotificationPolicy == MATCHED_ONLY`).

*Guards* -- <<type_DetectionMode,`DetectionMode`>> value in effect:

* `[REPEATING]` -- `DetectionMode == REPEATING`: card detection remains active after each detection.
* `[SINGLE_SHOT]` -- `DetectionMode == SINGLE_SHOT`: card detection stops after the first detection.
...
----
include::uml/class-diagram.puml[Terminal Reader API class diagram, align=center]
----
