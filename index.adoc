...
| Returns     | The current instance (<<type_CardDetectionSettings,`CardDetectionSettings`>>).
| Errors      | `IllegalArgumentError` -- if the provided array is empty.
|===
...
| Returns     | The current instance (<<type_CardDetectionSettings,`CardDetectionSettings`>>).
| Errors      | `IllegalArgumentError` -- if the provided set is empty.
|===
...
| Errors      | `IllegalArgumentError` -- if the supplied string is not a valid regular expression.
|===
...
| Exports the content of the previously processed selection scenario as a `String`. The result MAY be imported
  ...
  *Caution:* if the local environment does not have all the card extensions involved in the scenario, the
  ...
  card extensions.
| Returns     | A non-empty `String`.
| Errors      | `IllegalStateError` -- if the scenario has not yet been processed or has failed.
...
| Errors      | `IllegalArgumentError` -- if the string is malformed.
...
| Returns     | The current instance (`T`).
| Errors      | `IllegalArgumentError` -- if the provided regular expression is empty or invalid.
|===
...
| Errors      | `IllegalArgumentError` -- if the provided array is out of range.
|===
...
| Returns     | The current instance (<<type_IsoCardSelector,`IsoCardSelector`>>).
| Errors      | `IllegalArgumentError` -- if the provided string is invalid or out of range.
|===
...
| Errors      | None. Any error thrown by the handler is routed to `onReaderError`.
|===
...
| Errors      | <<type_InvalidCardResponse,`InvalidCardResponse`>> -- if the card does not support multi-channel.
|===
