...
| Exports the content of the previously processed selection scenario as a `String`. The result MAY be imported
  ...
  *Caution:* if the local environment does not have all the card extensions involved in the scenario, the
  ...
  card extensions.
| Returns        | A non-empty `String`.
| Pre-conditions | *State* -- the scenario has been processed successfully, via `processCardSelectionScenario`,
...
| Pre-conditions | *Argument* -- every parameter is non-`null`. +
  *Argument* -- the string has been produced by `exportProcessedCardSelectionScenario`, is well-formed and contains
  no more selection cases than the current scenario. +
