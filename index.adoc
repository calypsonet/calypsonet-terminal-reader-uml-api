...
| [[ref_cna_trd]] [CNA-TRD-API]
| https://docs.terminal-api.calypsonet.org/calypsonet-terminal-reader-definitions-uml-api/[_CNA Terminal API -- Reader Definitions_] (SP-CNATerminalAPI-ReaderDefinitions), version {ver-cna-trd}, {organization}. Provides the `RfTechnology` and `CardType` enumerations referenced by this specification.
|===
...
<<type_SequentialCardSelectionResult,`SequentialCardSelectionResult`>> or
...
<<type_SequentialCardSelectionManager,`SequentialCardSelectionManager`>> only -- and executed automatically by the
...
| `cardType`
| `CardType?` (<<ref_cna_trd,CNA-TRD-API>>)
| `null`
| 3.0
...
| `rfTechnologies`
| `Set<RfTechnology>` (<<ref_cna_trd,CNA-TRD-API>>)
...
| --
| 3.0
| Type of the detected card, as defined by the `CardType` enumeration of the Terminal Reader Definitions API
  (see <<ref_cna_trd,CNA-TRD-API>>). `UNKNOWN` when the card type could not be identified. The value is
...
| All <<type_SmartCard,`SmartCard`>> instances corresponding to successful selection cases, each active on its own
  logical channel, indexed by the `selectionCaseId` supplied to `prepareSelectionCase`. Possibly empty.
|===

[[type_SequentialCardSelectionResult]]
==== Sequential Card Selection Result

[cols="^.^1h,.^5", grid=rows, frame=all]
|===
| Name       | `SequentialCardSelectionResult`
| Kind       | Data
| Namespace  | `reader.selection`
| Since      | 3.0
| Purpose    | Result of a selection scenario executed by a <<type_SequentialCardSelectionManager,`SequentialCardSelectionManager`>>.
|===

Every selection case of the scenario has been tried. The result exposes the smart cards of all the successful
selection cases, indexed by their `selectionCaseId`. Since the card is operated on a single channel, only the smart card
...
`activeSelectionCaseId` is `null` if and only if `smartCards` is empty.
...
| All <<type_SmartCard,`SmartCard`>> instances corresponding to successful selection cases, indexed by the
  `selectionCaseId` supplied to `prepareSelectionCase`. Possibly empty.

| `activeSelectionCaseId`
| `Int?`
| --
| 3.0
...
identified by the `selectionCaseId` supplied to `prepareSelectionCase`. `selectionCaseId` and `smartCard` are either both
...
All the selection cases of a scenario target the *same card* presented to the reader. What a scenario
discriminates is, in practice, the *applications* hosted by that card -- for example the Calypso applications of a
multi-application card. The qualifiers *single*, *sequential* and *multichannel* in the names of the managers
therefore describe how the scenario is executed and how many selections are retained, never a number of cards. The
term *card* is kept throughout the API because a selection case also targets cards that host no application, such
as storage cards, where the selection targets the card itself.
...
| <<type_SequentialCardSelectionManager,`SequentialCardSelectionManager`>>
| Single-channel. Processes every selection case regardless of intermediate successes.
| Explicit or scheduled
| <<type_SequentialCardSelectionResult,`SequentialCardSelectionResult`>>
...
A scenario is considered *prepared* once at least one selection case has been appended via `prepareSelectionCase` or
...
| <<op_SingleCardSelectionManager_importProcessedCardSelectionScenario,`SingleCardSelectionManager.importProcessedCardSelectionScenario`>> +
  <<op_SequentialCardSelectionManager_importProcessedCardSelectionScenario,`SequentialCardSelectionManager.importProcessedCardSelectionScenario`>> +
...
[[op_CardSelectionManager_prepareSelectionCase]]
===== Prepare Selection Case

[cols="^.^1h,.^5", grid=rows, frame=all]
|===
| Signature
| `prepareSelectionCase(` +
`{nbsp}{nbsp}{nbsp}{nbsp}selectionCaseId: Int,` +
`{nbsp}{nbsp}{nbsp}{nbsp}cardSelector: CardSelector,` +
`{nbsp}{nbsp}{nbsp}{nbsp}cardSelectionExtension: CardSelectionExtension` +
`) -> Self`
| Since          | 2.0
| Description
| Appends a selection case, identified by `selectionCaseId`, to the scenario. Selection cases are executed in the order
...
| Parameters
| `selectionCaseId` -- the identifier of the selection case, chosen by the application. +
...
| Returns        | The current instance (`Self`).
| Pre-conditions | *Argument* -- every parameter is non-`null`. +
  *Argument* -- `selectionCaseId` is not already used by a selection case of the scenario.
| Errors         | None.
|===
...
| Since      | 3.0
| Purpose    | Card selection manager executing every selection case of the scenario, keeping each matching application of an ISO/IEC 7816-4 multi-channel card active simultaneously, each on its own logical channel. Out of scope for storage cards. An instance is obtained via <<op_ReaderApiFactory_createMultichannelCardSelectionManager,`ReaderApiFactory.createMultichannelCardSelectionManager`>>.
|===
...
| See also       | <<op_SingleCardSelectionManager_scheduleCardSelectionScenario,`SingleCardSelectionManager.scheduleCardSelectionScenario`>> +
                   <<op_SequentialCardSelectionManager_scheduleCardSelectionScenario,`SequentialCardSelectionManager.scheduleCardSelectionScenario`>>
|===
...
[[op_ReaderApiFactory_createSequentialCardSelectionManager]]
===== Create Sequential Card Selection Manager

[cols="^.^1h,.^5", grid=rows, frame=all]
|===
| Signature      | `createSequentialCardSelectionManager() -> SequentialCardSelectionManager`
| Since          | 3.0
| Description    | Creates a <<type_SequentialCardSelectionManager,`SequentialCardSelectionManager`>>, the service used to prepare and execute a card selection scenario processing every selection case.
| Returns        | A <<type_SequentialCardSelectionManager,`SequentialCardSelectionManager`>>.
| Pre-conditions | None.
| Errors         | None.
|===
...
[[type_SequentialCardSelectionManager]]
==== Sequential Card Selection Manager

[cols="^.^1h,.^5", grid=rows, frame=all]
|===
| Name       | `SequentialCardSelectionManager`
| Kind       | Interface
| Namespace  | `reader.selection`
| Extends    | <<type_CardSelectionManager,`CardSelectionManager`>>
| Since      | 3.0
| Purpose    | Card selection manager executing every selection case of the scenario, selecting in turn each matching application of the card; only the last selected application remains active. Intended for cards hosting several applications, and therefore not applicable to storage cards. An instance is obtained via <<op_ReaderApiFactory_createSequentialCardSelectionManager,`ReaderApiFactory.createSequentialCardSelectionManager`>>.
|===
...
the previous one: in the resulting <<type_SequentialCardSelectionResult,`SequentialCardSelectionResult`>>, only the
...
[[op_SequentialCardSelectionManager_importProcessedCardSelectionScenario]]
===== Import Processed Card Selection Scenario
...
| `importProcessedCardSelectionScenario(` +
`{nbsp}{nbsp}{nbsp}{nbsp}processedCardSelectionScenario: String` +
`) -> SequentialCardSelectionResult`
| Since          | 3.0
| Description    | Imports a previously exported *processed* selection scenario and returns the corresponding result.
| Parameters     | `processedCardSelectionScenario` -- the `String` containing the exported processed card selection scenario.
| Returns        | A <<type_SequentialCardSelectionResult,`SequentialCardSelectionResult`>>.
| Pre-conditions | *Argument* -- every parameter is non-`null`. +
  *Argument* -- the string has been produced by `exportProcessedCardSelectionScenario` of a `SequentialCardSelectionManager`, is
  well-formed and contains no more selection cases than the current scenario. +
  *State* -- the manager has been configured with the same selection scenario as the manager that produced the
  export. +
  *Capability* -- the local environment possesses every card extension involved in the scenario.
...
[[op_SequentialCardSelectionManager_parseScheduledCardSelectionsResponse]]
...
[[op_SequentialCardSelectionManager_processCardSelectionScenario]]
===== Process Card Selection Scenario
...
[[op_SequentialCardSelectionManager_scheduleCardSelectionScenario]]
...
| Schedules the execution of the prepared selection scenario as soon as a card is presented to the supplied
  ...
  <<op_SequentialCardSelectionManager_parseScheduledCardSelectionsResponse,`parseScheduledCardSelectionsResponse`>>.
| Parameters
...
| Extends    | <<type_CardSelectionManager,`CardSelectionManager`>>
| Since      | 3.0
| Purpose    | Card selection manager executing the scenario up to the first matching selection case: the first matching card application -- for example the Calypso application of a Calypso card -- or the card itself when it hosts no application, as a storage card. An instance is obtained via <<op_ReaderApiFactory_createSingleCardSelectionManager,`ReaderApiFactory.createSingleCardSelectionManager`>>.
|===
