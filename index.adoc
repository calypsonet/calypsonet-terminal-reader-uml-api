...
[cols="^.^1h,.^5", grid=rows, frame=all]
|===
| Signature   | `setEcpFrame(ecpFrame: ByteArray) -> CardDetectionSettings`
| Since       | 3.0
| Description
...
[cols="^.^1h,.^5", grid=rows, frame=all]
|===
| Signature   | `importCardSelectionScenario(cardSelectionScenario: String) -> Int`
| Since       | 1.1
...
| Returns      | A non-negative `Int` -- the index of the appended selection case.
| Throws      | None.
|===
...
[cols="^.^1h,.^5", grid=rows, frame=all]
|===
| Signature   | `getActiveSelectionIndex() -> Int`
| Since       | 1.0
...
| Returns     | A non-negative `Int` if an active selection exists, `-1` otherwise.
| Throws      | None.
|===
...
[cols="^.^1h,.^5", grid=rows, frame=all]
|===
| Signature   | `getActiveSelectionIndexes() -> List<Int>`
| Since       | 3.0
| Description | Returns the indexes of all active selections, one per channel in multi-channel mode.
| Returns     | A possibly empty list of non-negative `Int` values.
| Throws      | None.
|===
...
[cols="^.^1h,.^5", grid=rows, frame=all]
|===
| Signature   | `getSmartCards() -> Map<Int, SmartCard>`
| Since       | 1.0
...
[cols="^.^1h,.^5", grid=rows, frame=all]
|===
| Signature   | `filterByDfName(aid: ByteArray) -> IsoCardSelector`
| Since       | 2.0
| Description
...
| Parameters  | `aid` -- the AID as a `ByteArray` containing 5 to 16 bytes.
