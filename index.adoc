...
| Response of the selection scenario scheduled on the reader. It is `null` *only* when no selection
  ...
  called on the <<type_CardSelectionManager,`CardSelectionManager`>> instance that prepared the associated
  selection scenario, or on another `CardSelectionManager` sharing the same context -- that is, one that
  has previously prepared the same selection scenario.
|===
...
Each selection case prepared with the manager is associated with the index returned by `prepareSelection`.
The same index is used to retrieve the corresponding result here. In single-channel mode, at most one case
will correspond to the active selected card. In multi-channel mode, several active selections may coexist;
see `activeSelectionIndexes` below.
...
| Type of the detected card, as defined by the `CardType` enumeration of the Terminal Definitions API
  ...
  intrinsically tied to the lifecycle of this `CardSelectionResult`.
...
| All <<type_SmartCard,`SmartCard`>> instances corresponding to successful selection cases, indexed by the
  value returned by `prepareSelection`. Possibly empty.

| `activeSmartCard`
| <<type_SmartCard,`SmartCard`>>?
| --
| 1.0
| The active matching card, that is, the card that has been selected, or `null` if there is none. In case
  of multiple active smart cards (multi-channel mode), the one placed on channel `0`.

| `activeSelectionIndex`
| `Int?`
| --
| 1.0
| Index of the active selection, or `null` if there is none. Identical to the first element of
  `activeSelectionIndexes` when applicable.

| `activeSelectionIndexes`
| `List<Int>`
| --
| 3.0
| Indexes of all active selections, one per channel in multi-channel mode. Possibly empty. Values are
  non-negative.
|===
