...
| Returns     | The current instance (<<type_CardDetectionSettings,`CardDetectionSettings`>>).
| Throws      | `IllegalArgumentError` -- if the provided array is empty.
|===
...
| Returns     | The current instance (<<type_CardDetectionSettings,`CardDetectionSettings`>>).
| Throws      | `IllegalArgumentError` -- if the provided set is empty.
|===
...
| Throws      | <<type_ReaderCommunication,`ReaderCommunication`>> -- if the communication with the reader has failed.
|===
...
| Throws      | `IllegalArgumentError` -- if the supplied string is not a valid regular expression.
|===
...
| Exports the content of the previously processed selection scenario as a `String`. The result MAY be imported
  ...
  *Caution:* if the local environment does not have all the card extensions involved in the scenario, the
  ...
  card extensions.
| Returns     | A non-empty `String`.
| Throws      | `IllegalStateError` -- if the scenario has not yet been processed or has failed.
...
| Throws      | `IllegalArgumentError` -- if the string is malformed.
...
| Throws
| `IllegalArgumentError` -- if the string is malformed or contains more selection cases than the
  current scenario. +
  <<type_InvalidCardResponse,`InvalidCardResponse`>> -- if the data returned by the card could not be interpreted.
...
| Returns     | A <<type_CardSelectionResult,`CardSelectionResult`>>.
| Throws      | <<type_InvalidCardResponse,`InvalidCardResponse`>> -- if the data returned by the card could not be interpreted.
|===
...
| Explicitly executes the previously prepared selection scenario in *multi-channel* mode against the provided
  ...
  *Prerequisite:* the presented card MUST be a multi-channel ISO/IEC 7816-4 card. If it is not, an
  `InvalidCardResponse` is raised at scenario execution time.
| Parameters
| `reader` (<<type_CardReader,`CardReader`>>)                 -- the reader through which the card is reached. +
...
| Returns     | The current instance (`T`).
| Throws      | `IllegalArgumentError` -- if the provided regular expression is empty or invalid.
|===
...
| Throws      | `IllegalArgumentError` -- if the provided array is out of range.
|===
...
| Returns     | The current instance (<<type_IsoCardSelector,`IsoCardSelector`>>).
| Throws      | `IllegalArgumentError` -- if the provided string is invalid or out of range.
|===
...
| Activates card detection. Once activated, the application is notified of card events through the registered
  ...
  A `WARN`-level message is typically logged in such cases. No error is thrown.
| Parameters
...
| `context`     -- the context describing the operation that failed. +
  ...
  `e`           -- the original error that triggered the notification.
| Throws       | None.
|===
...
| Throws      | None. Any error thrown by the handler is routed to `onReaderError`.
|===
...
| <<type_ReaderCommunication,`ReaderCommunication`>> -- if a communication error with the reader occurs. +
  ...
  <<type_InvalidCardResponse,`InvalidCardResponse`>> -- if a command returns an unexpected status.
|===
...
| Returns a view of this manager as a <<type_MultichannelCardTransactionManager,
  ...
  *Prerequisite:* the underlying ISO/IEC 7816-4 card MUST support multi-channel operation; otherwise an `InvalidCardResponse` is raised during command processing.
| Returns     | A <<type_MultichannelCardTransactionManager,`MultichannelCardTransactionManager`>>.
| Throws      | <<type_InvalidCardResponse,`InvalidCardResponse`>> -- if the card does not support multi-channel.
|===
