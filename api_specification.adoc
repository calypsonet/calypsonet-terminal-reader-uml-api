...
| Returns whether this smart card is currently active on its logical channel.
  ...
  occurrence of any of the following events, at which point the reader MUST *deactivate* it (`isActive()` returns
  `false`) and release its reference:
  ...
  Until then, the reader MUST hold the references to the `SmartCard` instances produced by the last successful
  selection. Once a smart card has been deactivated, any further attempt to use it MUST be detected by the
